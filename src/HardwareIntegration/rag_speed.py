...
TARGET SPEED:
- Desired speed is 11 m/s

STRICT TOKEN RULES:
If the distance to the speed change zone is less than or equal to 25 meters, and the velocity is either greater than 12 m/s or less than 11 m/s, then initiate actuation.
If the distance to the speed change zone is between 50 and 60 meters, and the velocity is outside the range of 13 to 15 m/s, then issue a warning.
If the distance to the speed change zone is greater than 80 meters, and the velocity is outside the range of 16 to 18 m/s, then issue a warning.
...
Do not output anything else and keep it natural
"""
